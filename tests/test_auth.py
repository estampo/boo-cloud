"""Tests for boocloud.auth — login flows, profile, device discovery."""
...
class TestLoginPasswordFlow:
    def test_direct_password_success(self):
        resp = _mock_response(
            200,
            json_data={
                "accessToken": "tok123",
    ...
    def test_no_token_and_no_special_flow_raises(self):
        resp = _mock_response(200, json_data={"loginType": "unknown", "message": "something odd"})
        with patch("requests.post", return_value=resp):
            with pytest.raises(RuntimeError, match="Login failed"):
                _login("user@example.com", "pw")

    def test_missing_refresh_token_defaults_empty(self):
        resp = _mock_response(200, json_data={"accessToken": "tok", "loginType": "password"})
        with patch("requests.post", return_value=resp):
...
class TestLoginVerificationCodeFlow:
    def test_verify_code_flow(self, monkeypatch):
        monkeypatch.setattr("boocloud.auth.getpass.getpass", lambda prompt: "123456")
    ...
    def test_verify_code_flow_no_token_raises(self, monkeypatch):
        monkeypatch.setattr("boocloud.auth.getpass.getpass", lambda prompt: "000000")
...
class TestLoginTfaFlow:
    def test_tfa_flow(self, monkeypatch):
        monkeypatch.setattr("boocloud.auth.getpass.getpass", lambda prompt: "654321")
    ...
    def test_tfa_http_error(self, monkeypatch):
        monkeypatch.setattr("boocloud.auth.getpass.getpass", lambda prompt: "123")
...
class TestGetUserProfile:
    def test_returns_profile_dict(self):
        ...
        assert profile == {"uid": "12345", "name": "Alice", "avatar": "https://img/a.png"}
